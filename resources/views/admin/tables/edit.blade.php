                ...
                <div class="space-y-8 divide-y divide-gray-200 w-1/2 mt-10">
                    <form method="POST" action="{{ route('admin.tables.update', $table->id) }}">
                        ...
                        <div class="sm:col-span-6 pt-2">
                            ...
                            <div class="mt-1">
                                <input type="number" min="0" max="10000" id="guest_number"
                        ...
                        <div class="sm:col-span-6 pt-2">
                            ...
                            <div class="mt-1">
                                <select name="status"
                                    ...
                                    @foreach (App\Enums\TableStatus::cases() as $data)
                                        <option value={{ $data->value }} @selected($table->status->value == $data->value)>
                                            {{ $data->name }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="sm:col-span-6 pt-2">
                            ...
                            <div class="mt-1">
                                <select name="location"
                                    class="shadow-sm focus:ring-indigo-500 appearance-none bg-white border py-2 px-3 text-base leading-normal transition duration-150 ease-in-out focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md">
                                    @foreach (App\Enums\TableLocation::cases() as $data)
                                    <option value={{ $data->value }} @selected($table->location->value == $data->value)>
                                        {{ $data->name }}</option>
